...
static u64_t core_pll_clk_get_rate(struct clk_pll_t * pclk, u64_t prate)
{
	...
	if(strcmp(pclk->name, "PLL0") == 0)
	{
		 ...
		 */
		r = read32(phys_to_virt(S5P6818_SYS_PLLSETREG0));
		p = (r >> 18) & 0x3f;
		m = (r >> 8) & 0x3ff;
		s = (r >> 0) & 0xff;
		fout = m * (prate / (p * (1 << s)));
	...
	else if(strcmp(pclk->name, "PLL1") == 0)
	{
		 ...
		 * FOUT = MDIV * FIN / (PDIV * 2^(SDIV))
		 */
		r = read32(phys_to_virt(S5P6818_SYS_PLLSETREG1));
		p = (r >> 18) & 0x3f;
		m = (r >> 8) & 0x3ff;
		s = (r >> 0) & 0xff;
		fout = m * (prate / (p * (1 << s)));
	...
	else if(strcmp(pclk->name, "PLL2") == 0)
	{
		 ...
		 */
		r = read32(phys_to_virt(S5P6818_SYS_PLLSETREG2));
		p = (r >> 18) & 0x3f;
		m = (r >> 8) & 0x3ff;
		s = (r >> 0) & 0xff;
		r = read32(phys_to_virt(S5P6818_SYS_PLLSETREG2_SSCG));
		k = (r >> 16) & 0xffff;
	...
	else if(strcmp(pclk->name, "PLL3") == 0)
	{
		 ...
		 */
		r = read32(phys_to_virt(S5P6818_SYS_PLLSETREG3));
		p = (r >> 18) & 0x3f;
		m = (r >> 8) & 0x3ff;
		s = (r >> 0) & 0xff;
		r = read32(phys_to_virt(S5P6818_SYS_PLLSETREG3_SSCG));
		k = (r >> 16) & 0xffff;
		fout = (m + k / 65536) * (prate / (p * (1 << s)));
	}
...
static struct clk_mux_t core_mux_clks[] = {
	{
		...
		.reg = S5P6818_SYS_CLKDIVREG0,
		.shift = 0,
		.width = 3,
	}, {
		.name = "MUX-BCLK",
		.parent = pll_mux_tables,
		.reg = S5P6818_SYS_CLKDIVREG1,
		.shift = 0,
		.width = 3,
	}, {
		.name = "MUX-MDCLK",
		.parent = pll_mux_tables,
		.reg = S5P6818_SYS_CLKDIVREG2,
		.shift = 0,
		.width = 3,
	}, {
		.name = "MUX-GR3DBCLK",
		.parent = pll_mux_tables,
		.reg = S5P6818_SYS_CLKDIVREG3,
		.shift = 0,
		.width = 3,
	}, {
		.name = "MUX-MPEGBCLK",
		.parent = pll_mux_tables,
		.reg = S5P6818_SYS_CLKDIVREG4,
		.shift = 0,
		.width = 3,
	}
};
...
static struct clk_mux_t uart_mux_clks[] = {
	{
		...
		.reg = S5P6818_CLK_UART0CLKGEN0L,
		.shift = 2,
		.width = 3,
	}, {
		.name = "MUX-UART1",
		.parent = uart_mux_tables,
		.reg = S5P6818_CLK_UART1CLKGEN0L,
		.shift = 2,
		.width = 3,
	}, {
		.name = "MUX-UART2",
		.parent = uart_mux_tables,
		.reg = S5P6818_CLK_UART2CLKGEN0L,
		.shift = 2,
		.width = 3,
	}, {
		.name = "MUX-UART3",
		.parent = uart_mux_tables,
		.reg = S5P6818_CLK_UART3CLKGEN0L,
		.shift = 2,
		.width = 3,
	}, {
		.name = "MUX-UART4",
		.parent = uart_mux_tables,
		.reg = S5P6818_CLK_UART4CLKGEN0L,
		.shift = 2,
		.width = 3,
	}, {
		.name = "MUX-UART5",
		.parent = uart_mux_tables,
		.reg = S5P6818_CLK_UART5CLKGEN0L,
		.shift = 2,
		.width = 3,
	}
};
...
static struct clk_gate_t uart_gate_clks[] = {
	{
		.name = "GATE-UART0",
		.parent = "DIV-UART0",
		.reg = S5P6818_CLK_UART0CLKENB,
		.shift = 1,
		.invert = 0,
	}, {
		.name = "GATE-UART1",
		.parent = "DIV-UART1",
		.reg = S5P6818_CLK_UART1CLKENB,
		.shift = 1,
		.invert = 0,
	}, {
		.name = "GATE-UART2",
		.parent = "DIV-UART2",
		.reg = S5P6818_CLK_UART2CLKENB,
		.shift = 1,
		.invert = 0,
	}, {
		.name = "GATE-UART3",
		.parent = "DIV-UART3",
		.reg = S5P6818_CLK_UART3CLKENB,
		.shift = 1,
		.invert = 0,
	}, {
		.name = "GATE-UART4",
		.parent = "DIV-UART4",
		.reg = S5P6818_CLK_UART4CLKENB,
		.shift = 1,
		.invert = 0,
	}, {
		.name = "GATE-UART5",
		.parent = "DIV-UART5",
		.reg = S5P6818_CLK_UART5CLKENB,
		.shift = 1,
		.invert = 0,
	}
};
...
void s5p6818_clk_init(void)
{
	...
	clk_set_rate("DIV-UART0", 375 * 100 * 1000);
	clk_set_rate("DIV-UART1", 375 * 100 * 1000);
	clk_set_rate("DIV-UART2", 375 * 100 * 1000);
	clk_set_rate("DIV-UART3", 375 * 100 * 1000);
	clk_set_rate("DIV-UART4", 375 * 100 * 1000);
	clk_set_rate("DIV-UART5", 375 * 100 * 1000);
