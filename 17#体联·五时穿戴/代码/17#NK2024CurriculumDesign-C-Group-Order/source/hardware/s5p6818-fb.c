...
struct s5p6818_fb_data_t
{
	...
	/* Init and exit */
	void (*init)(struct s5p6818_fb_data_t * dat);
	void (*exit)(struct s5p6818_fb_data_t * dat);
	/* Set and get backlight brightness */
	void (*setbl)(struct s5p6818_fb_data_t * dat, int brightness);
	int (*getbl)(struct s5p6818_fb_data_t * dat);
...
{
	struct s5p6818_fb_data_t * dat = (struct s5p6818_fb_data_t *)(fb->dat);
...
static void fb_flush(struct fb_t * fb)
{
	...
	s5p6818_mlc_set_layer_address(dat, 0, dat->vram_front);
	s5p6818_mlc_set_dirty_flag(dat, 0);
}
...
static struct fb_t s5p6818_fb;

void s5p6818_fb_initial(void)
{
	struct s5p6818_fb_data_t * dat = &s5p6818_fb_data;

	if( (dat->bits_per_pixel != 16) && (dat->bits_per_pixel != 24) && (dat->bits_per_pixel != 32) )
		return;

	s5p6818_fb.dat = dat;
	s5p6818_fb.surface.info.bits_per_pixel = dat->bits_per_pixel;
	s5p6818_fb.surface.info.bytes_per_pixel = dat->bytes_per_pixel;
	s5p6818_fb.surface.info.red_mask_size = dat->rgba.r_mask;
	s5p6818_fb.surface.info.red_field_pos = dat->rgba.r_field;
	s5p6818_fb.surface.info.green_mask_size = dat->rgba.g_mask;
	s5p6818_fb.surface.info.green_field_pos = dat->rgba.g_field;
	s5p6818_fb.surface.info.blue_mask_size = dat->rgba.b_mask;
	s5p6818_fb.surface.info.blue_field_pos = dat->rgba.b_field;
	s5p6818_fb.surface.info.alpha_mask_size = dat->rgba.a_mask;
	s5p6818_fb.surface.info.alpha_field_pos = dat->rgba.a_field;
	s5p6818_fb.surface.info.fmt = get_pixel_format(&(s5p6818_fb.surface.info));

	s5p6818_fb.surface.w = dat->width;
	s5p6818_fb.surface.h = dat->height;
	s5p6818_fb.surface.pitch = dat->width * dat->bytes_per_pixel;
	s5p6818_fb.surface.flag = SURFACE_PIXELS_DONTFREE;
	s5p6818_fb.surface.pixels = dat->vram_front;

	s5p6818_fb.surface.clip.x = 0;
	s5p6818_fb.surface.clip.y = 0;
	s5p6818_fb.surface.clip.w = dat->width;
	s5p6818_fb.surface.clip.h = dat->height;

	memset(&s5p6818_fb.surface.maps, 0, sizeof(struct surface_maps));
	surface_set_maps(&s5p6818_fb.surface.maps);
...
{
	fb_backlight(&s5p6818_fb, brightness);
}
