/****************************************************
 *XX#NK2024CurriculumDesign-C-Group-Order-template.c*
 ****************************************************
 * 代码部分整体操作说明：									*
 * 1.代码构成：											*
 ...
 *    -注释填充：NK2024F-XX#-Le/R/Lo/O/S				*
 *    -注释替换：NK2024R-XX#-Le/R/Lo/O/S				*
 *    -注释增删：NK2024I&D-XX#-Le/R/Lo/O/S				*
 ***************************************************/

/*********************认  知  发  问  部  分********************
 *
 *【问题定位（示例）】Q0(NK2024-01#-Le)
 *【问题描述（示例）】tester_key_led_beep函数在哪里被调用的？
 *【解决状态（示例）】已解决(NK2024-01#-R)
...
void show_group_number()
{
	...
	led_set_status(LED_NAME_LED4, LED_STATUS_ON);  	//	1(10)=0001(2),亮最低位的一个灯												// 0
	...
	led_set_status(LED_NAME_LED1, LED_STATUS_OFF); 															// 0
	led_set_status(LED_NAME_LED2, LED_STATUS_ON); 															// 0
	led_set_status(LED_NAME_LED3, LED_STATUS_OFF); 															// 0
	led_set_status(LED_NAME_LED4, LED_STATUS_ON); 	//5(10)=0101(2),亮第二位和第四位														// 0
